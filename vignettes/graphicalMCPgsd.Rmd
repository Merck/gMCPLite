...
for (i in 1:nHypotheses) {
  ...
  if (!is.null(gsDlist[[i]])) {
    ...
    if (EOCtab$lastAlpha[i] > 0) {
      ...
      bound_tbl <- tibble(
        Analysis = seq_along(n.I),
        Z = dupdate$upper$bound,
        `Spending time` = usTime,
        `Nominal p bound` = pnorm(-dupdate$upper$bound)
      )
      ...
      for (aa in seq_len(n_analyses)) {
        ...
        seq_p_by_analysis[aa] <- sequentialPValue(
          gsD = d, interval = c(.0001, .9999),
          n.I = n.I,
          Z = z_seq,
          usTime = usTime
        )
        ...
        z_rep[aa] <- all_z[aa]
        rep_p_by_analysis[aa] <- sequentialPValue(
          gsD = d, interval = c(.0001, .9999),
          n.I = n.I,
          Z = z_rep,
          usTime = usTime
