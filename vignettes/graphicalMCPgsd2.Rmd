...
- Sequential p-values are computed using **gsDesign2** (development branch `605-build-sequential-p-values-for-a-gsdesign2-object`).
...
library(gsDesign2) # Sequential p-values for gsDesign2 objects (dev branch)
library(graphicalMCP) # Multiplicity evaluation and graph updates

if (!"sequential_pval" %in% getNamespaceExports("gsDesign2")) {
  stop(
    "This vignette requires a development version of gsDesign2 that exports `sequential_pval()`.\n",
    "Install branch `605-build-sequential-p-values-for-a-gsdesign2-object` from `Merck/gsDesign2`."
  )
}
```
...
# A parallel list of gsDesign2 objects for sequential p-values.
# These are used only to compute sequential p-values via gsDesign2::sequential_pval().
# The gsDesign2 objects are constructed with minimal inputs, and we override event and spending time.
make_gsd2_seq <- function(k, alpha, binding = FALSE, sf = gsDesign::sfLDOF, sfpar = -4) {
  alpha_obj <- max(alpha, 1e-05)
  gsDesign2::gs_design_ahr(
    enroll_rate = gsDesign2::define_enroll_rate(duration = 1, rate = 1),
    fail_rate = gsDesign2::define_fail_rate(duration = 1, fail_rate = 0.1, hr = 0.7, dropout_rate = 0),
    alpha = alpha_obj,
    beta = 0.2,
    analysis_time = seq_len(k),
    binding = binding,
    upper = gsDesign2::gs_spending_bound,
    upar = list(sf = sf, param = sfpar, total_spend = alpha_obj),
    test_lower = FALSE
  )
}

gsD2list <- list(
  make_gsd2_seq(k = 3, alpha = alphaHypotheses[1], binding = FALSE),
  make_gsd2_seq(k = 3, alpha = alphaHypotheses[2], binding = FALSE),
  make_gsd2_seq(k = 2, alpha = alphaHypotheses[3], binding = FALSE),
  make_gsd2_seq(k = 2, alpha = alphaHypotheses[4], binding = FALSE),
  NULL,
  NULL
)
```
