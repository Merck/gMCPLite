...
In this vignette we use `gsDesign2::gs_design_ahr()` to create time-to-event (OS/PFS) group sequential designs and `gsDesign2::gs_update_ahr()` to update design bounds at the time of analysis.
To align the sequential p-values with the `rpact` vignette, we use `info_scale = "h0_info"` and treat the (blinded) information fraction as spending time, \(t_k = n_k / n_K\).
...
```{r}
osmedian <- 12 # Median control survival (for context)

ossub <- gsDesign2::gs_design_ahr(
  alpha = alphaHypotheses[1],
  beta = 0.1,
  binding = FALSE,
  analysis_time = c(0.61, 0.82, 1),
  info_frac = c(0.61, 0.82, 1),
  info_scale = "h0_info",
...
gsDesign2::gs_bound_summary(ossub) %>%
  gt::gt() %>%
  gt::tab_header(title = "Design for OS in the Subgroup (gsDesign2)")
```
...
os <- gsDesign2::gs_design_ahr(
  alpha = 0.01,
  beta = beta,
  binding = FALSE,
  analysis_time = c(0.62, 0.83, 1),
  info_frac = c(0.62, 0.83, 1),
  info_scale = "h0_info",
  upper = gsDesign2::gs_spending_bound,
  upar = list(sf = gsDesign::sfLDOF, total_spend = 0.01),
  test_lower = FALSE
)

gsDesign2::gs_bound_summary(os) %>%
  gt::gt() %>%
  gt::tab_header(title = "Design for OS in All subjects (gsDesign2)")
```
...
pfssub <- gsDesign2::gs_design_ahr(
  alpha = 0.004,
  beta = beta,
  binding = FALSE,
  analysis_time = c(0.87, 1),
  info_frac = c(0.87, 1),
  info_scale = "h0_info",
  upper = gsDesign2::gs_spending_bound,
  upar = list(sf = gsDesign::sfLDOF, total_spend = 0.004),
  test_lower = FALSE
)

gsDesign2::gs_bound_summary(pfssub) %>%
  gt::gt() %>%
  gt::tab_header(title = "Design for PFS in the Subgroup (gsDesign2)")
```
...
pfs <- gsDesign2::gs_design_ahr(
  alpha = 0.004,
  beta = beta,
  binding = FALSE,
  analysis_time = c(0.86, 1),
  info_frac = c(0.86, 1),
  info_scale = "h0_info",
  upper = gsDesign2::gs_spending_bound,
  upar = list(sf = gsDesign::sfLDOF, total_spend = 0.004),
...
gsDesign2::gs_bound_summary(pfs) %>%
  gt::gt() %>%
  gt::tab_header(title = "Design for PFS in the Overall population (gsDesign2)")
```
